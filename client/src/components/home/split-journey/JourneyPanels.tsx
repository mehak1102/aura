...
export function JourneyCopyPanel({ slides, total, className }: JourneyCopyProps) {
  return (
    <div
      data-journey-copy
      className={cn('relative h-full w-full bg-[#3e4736]', className)}
    >
      {slides.map((slide, i) => {
        ...
        return (
          <div
            ...
            data-index={i}
            className="absolute inset-0 flex flex-col justify-center px-[clamp(1.75rem,4.5vw,4rem)] py-10"
            style={{
              opacity: i === 0 ? 1 : 0,
          ...
          >
            <div data-journey-meta="" className="flex items-center gap-3">
              <span className="font-display text-[0.95rem] tabular-nums tracking-wide text-[#e8dcc8]">
                {slide.step}
              </span>
              <span aria-hidden className="h-px w-8 bg-[#d4b87a]/70" />
              <span className="text-[0.55rem] font-medium tracking-[0.28em] uppercase text-[#d4b87a]/85">
            ...
            <p
              data-journey-meta=""
              className="mt-6 text-[0.62rem] font-medium tracking-[0.32em] uppercase text-[#d4b87a]"
            >
              {slide.label}
            </p>

            <h3 className="mt-3 max-w-[14ch] font-display text-[clamp(1.85rem,3.1vw,2.85rem)] leading-[1.08] tracking-tight text-[#f7f3ea] text-pretty">
              <TitleWords lines={slide.titleLines} italic={slide.titleItalic} />
            </h3>

            <div className="mt-5 max-w-[28rem] space-y-0">
              {descLines.map((line) => (
                <p
                  key={line}
                  data-journey-line=""
                  className="text-[0.95rem] font-light leading-[1.75] text-[#ebe6dc] text-pretty"
                >
                  {line}
                </p>
              ))}
            ...
            <p
              data-journey-meta=""
              className="mt-4 text-[0.58rem] font-medium tracking-[0.18em] uppercase text-[#d4b87a]"
            >
              {slide.note}
            </p>

            <div data-journey-meta="" className="mt-8">
              <Link
                to={slide.cta.to}
                className="group/cta inline-flex h-12 items-center gap-2.5 rounded-full border border-[#d4b87a]/70 bg-transparent px-7 text-[0.62rem] font-semibold tracking-[0.2em] uppercase !text-white transition-colors duration-400 hover:border-[#d4b87a] hover:bg-[#d4b87a]/18 hover:!text-white"
                style={{ color: '#ffffff' }}
              >
                {slide.cta.label}
                <ArrowRight
      ...
      {slides.map((slide) => (
        ...
        >
          ...
          <div className="px-6 py-7">
            ...
            <h3 className="mt-3 font-display text-[1.55rem] leading-[1.1] tracking-tight text-[#f7f3ea]">
              <TitleWords lines={slide.titleLines} italic={slide.titleItalic} />
            </h3>
            <p className="mt-3 text-[0.9rem] font-light leading-[1.7] text-[#ebe6dc]">
              {slide.description}
            </p>
